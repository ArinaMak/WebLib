...
index.html - работа над фронтендом
My_servlet.java (My_servlet.class) - подключение к Базе Данных
my_base_class.java(my_base_class.cass) - действие, которое обрабатывается на данном сервесе (обработка кнопок)
index3.html и reg.jsp - первый взгляд на frontend и регистрацию
